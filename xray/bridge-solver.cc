...
// Global iteration counter for profiling (works in release mode)
static long long g_iteration_count = 0;
...
struct Options {
  ...
  int xray_iterations = 0;  // X-ray tracing: log first N calls to SearchWithCache
...
// X-ray tracing counter
static int xray_counter = 0;
...
struct Trick {
  ...
  bool IsEquivalent(int card, Cards tried_suit_cards, Cards hand) const {
    ...
    if (options.xray_iterations > 0 && xray_counter <= options.xray_iterations) {
      fprintf(stderr, "EQUIV_V2: card=%s tried=0x%llx hand=0x%llx all=0x%llx -> %s\n",
              NameOf(card), tried_suit_cards.Value(), hand.Suit(SuitOf(card)).Value(),
              all_cards.Suit(SuitOf(card)).Value(), result ? "true" : "false");
    }
    return result;
  }
...
class Play {
  ...
  Result SearchWithCache(int beta) {
    ...
    if (options.xray_iterations > 0 && xray_counter <= options.xray_iterations) {
      ...
      fprintf(stderr, "HANDS: W=%llx N=%llx E=%llx S=%llx\n",
              (unsigned long long)hands[WEST].Value(),
              (unsigned long long)hands[NORTH].Value(),
              (unsigned long long)hands[EAST].Value(),
              (unsigned long long)hands[SOUTH].Value());
 ...
 private:
  Result SearchAtTrickStart(int beta) {
    if (!options.no_pruning) {
      auto [fast_tricks, fast_rank_winners] = FastTricks();
      if (options.xray_iterations > 0 && xray_counter <= options.xray_iterations) {
        fprintf(stderr, "FAST_TRICKS: depth=%d seat=%d fast=%d trump=%d\n",
                depth, seat_to_play, fast_tricks, trump);
      }
      ...
      if (options.xray_iterations > 0 && xray_counter <= options.xray_iterations) {
        fprintf(stderr, "SLOW_TRICKS: depth=%d seat=%d slow=%d trump=%d\n",
                depth, seat_to_play, slow_tricks, trump);
      }
  ...
  Result EvaluatePlayableCards(int beta) {
    ...
    if (options.xray_iterations > 0 && xray_counter <= options.xray_iterations) {
      fprintf(stderr, "MOVE_ORDER_BEFORE: iter=%lld depth=%d play=[", g_iteration_count, depth);
      for (int d = 0; d < depth; ++d) {
        if (d > 0 && (d & 3) == 0) fprintf(stderr, " |");
        if (d > 0) fprintf(stderr, " ");
        fprintf(stderr, "%s", NameOf(plays[d].card_played));
      }
      fprintf(stderr, "] playable=[");
      bool first = true;
      for (int card : playable_cards) {
        if (!first) fprintf(stderr, " ");
        first = false;
        fprintf(stderr, "%s", NameOf(card));
      }
      fprintf(stderr, "] cutoff_cards=[");
      first = true;
      for (int card : cutoff_cards) {
        if (!first) fprintf(stderr, " ");
        first = false;
        fprintf(stderr, "%s", NameOf(card));
      }
    ...
    if (options.xray_iterations > 0 && xray_counter <= options.xray_iterations) {
      ...
      for (int i = 0; i < ordered_cards.Size(); ++i) {
        ...
        fprintf(stderr, "%s", NameOf(ordered_cards.Card(i)));
      }
    ...
    // ORDERED logging
    if (options.xray_iterations > 0 && xray_counter <= options.xray_iterations) {
      fprintf(stderr, "ORDERED: depth=%d playable=%llx count=%d cards=[",
              depth, original_playable.Value(), ordered_cards.Size());
      for (int i = 0; i < ordered_cards.Size(); ++i) {
        if (i > 0) fprintf(stderr, " ");
        fprintf(stderr, "%s", NameOf(ordered_cards.Card(i)));
      }
    ...
    for (int i = 0; i < ordered_cards.Size(); ++i) {
      ...
      if ((options.no_rank_skip || rank >= min_relevant_ranks[suit]) &&
        ...
        if (NsToPlay() ? ns_tricks >= beta : ns_tricks < beta) {  // cut-off
          ...
          if (options.xray_iterations > 0 && xray_counter <= options.xray_iterations) {
            fprintf(stderr, "CUTOFF: depth=%d seat=%d card=%s score=%d best=%d beta=%d maximizing=%s play=[",
                    depth, seat_to_play, NameOf(card),
                    branch_ns_tricks, ns_tricks, beta,
                    NsToPlay() ? "true" : "false");
            for (int d = 0; d < depth; ++d) {
              if (d > 0 && (d & 3) == 0) fprintf(stderr, " |");
              fprintf(stderr, " %s", NameOf(plays[d].card_played));
        ...
        int old_min = min_relevant_ranks[suit];
        ...
        // RANK_UPDATE logging
        if (options.xray_iterations > 0 && xray_counter <= options.xray_iterations &&
            min_relevant_ranks[suit] != old_min) {
          fprintf(stderr, "RANK_UPDATE: depth=%d card=%s suit=%d old_min=%d new_min=%d suit_rank_winners=%s play=[",
                  depth, NameOf(card), suit, old_min, min_relevant_ranks[suit],
